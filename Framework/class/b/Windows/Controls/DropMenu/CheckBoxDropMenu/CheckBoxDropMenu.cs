	...
	namespace Customs
	{
		...
		public bool Integrator(Control Parent, Point Location, Color BackColor, int MenuWidth = 110)
		{
			...
			Sorters.SortCode(("Base"), () =>
			{
			    ...
			    Integrate.Panel(Parent, Layer1, Size, Location, BackColor);
		...
		// Integrator 2:
		readonly List<CheckBox> DropMenuBoxes = new List<CheckBox>();
		readonly List<Entry> DropMenuItems = new List<Entry>();
		...
		Point GetNextItemLocation()
		{
			...
			if (DropMenuItems.Count > 0)
			{
			    Entry Entry = DropMenuItems[DropMenuItems.Count - 1];
